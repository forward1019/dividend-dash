...
import { tickerToCik } from '../ingest/sec-edgar.ts';
...
/**
 * Fetch the most recent 10-K for `ticker`. Returns null if no 10-K is found
 * (e.g. an ETF, which files N-CSR/N-1A instead).
 */
export async function fetchLatest10K(ticker: string): Promise<TenKDocument | null> {
  const t = ticker.toUpperCase();
  const cik = await tickerToCik(t);
  if (!cik) {
    throw new Error(`Could not resolve CIK for ${t}`);
  }
